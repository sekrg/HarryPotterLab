...
public class HarryPotter extends Person implements Ifeel, Italk, Ifound, Iretrieved, Irotate, Isaw, Istand, Ibent, Ibreathe, Icome, Iopen, Ipull, Iscarred, Ishove, Iwant, Ifly {
    ...
    public void retrieved(MagicObjects x) {
        System.out.println("Это его " + getName() + " извлек тогда в подземелье из " + x.toString() + ". ");
    }
    ...
    @Override
    public void scarred() {
        if(Math.random()>=0.3) throw new HeartStopException("Сердце у " + getName() + " сжалось от увиденного. Гарри Поттер умер от остановки сердца");
